...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .navbar { display: flex; align-items: center; justify-content: space-between; padding: 0.75rem 2rem; background: var(--navbar-bg); backdrop-filter: blur(20px); border-bottom: 1px solid var(--navbar-border); position: sticky; top: 0; z-index: 100; }
        ...
        .btn-logout { background: none; border: 1.5px solid rgba(255, 255, 255, 0.5); color: var(--text-primary); padding: 0.4rem 1rem; border-radius: 50px; font-size: 0.82rem; font-family: 'Poppins', sans-serif; font-weight: 500; cursor: pointer; transition: all 0.3s ease; }
...
<body class="{{ auth()->check() && auth()->user()->dark_mode ? 'dark-mode' : '' }}">
    ...
    <nav class="navbar">
        ...
        <ul class="navbar-nav">
            ...
            <li><a href="{{ route('interets.index') }}" class="nav-link {{ request()->routeIs('interets.*') ? 'active' : '' }}">
        ...
        <div class="navbar-user">
            ...
            <form method="POST" action="{{ route('logout') }}">@csrf <button type="submit" class="btn-logout">{{ __('app.logout') }}</button></form>
        </div>
    </nav>
